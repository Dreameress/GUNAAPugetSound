...
namespace GUNAAPugetSound.Services
{
   public interface IIdentityService  
    ...
    {  
        ...
        public IdentityService(GUNAADbContext context,  
        ...
        {  
            ...
            try
            {
                ApplicationHelper.CreatePasswordHash(login.Password, out var passwordHash, out var passwordSalt);
                ApplicationUsersMaster loginUser = _context.ApplicationUsersMaster.FirstOrDefault(c => c.UserName == login.UserName && c.PasswordHash == passwordHash);  
        ...
        private List<ApplicationRolesMaster> GetUserRole(long UserId)  
        {  
            try  
            {  
                List<ApplicationRolesMaster> rolesMasters = (from um in _context.ApplicationUsersMaster  
            ...
            {  
                return new List<ApplicationRolesMaster>();  
            }  
        }  
  
        public async Task<AuthenticationResult> AuthenticateAsync(ApplicationUsersMaster user)  
        {  
            ...
            {  
                ...
                await _context.UsersRefreshToken.AddAsync(refreshToken);  
        ...
        private async Task<AuthenticationResult> GetRefreshTokenAsync(string token, string refreshToken)
        {
            ...
            var storedRefreshToken = _context.UsersRefreshToken.FirstOrDefault(x => x.Token == refreshToken);
            ...
            _context.UsersRefreshToken.Update(storedRefreshToken);
            await _context.SaveChangesAsync();
            string strUserId = validatedToken.Claims.Single(x => x.Type == "UserId").Value;
            long.TryParse(strUserId, out var userId);
            var user = _context.ApplicationUsersMaster.FirstOrDefault(c => c.UserId == userId);
            if (user == null)
            {
